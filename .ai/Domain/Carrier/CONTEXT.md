...
Manages shipping carriers, their shipping ranges (by price and weight), zones, and delivery methods. Does not handle order shipping state or tracking — those belong to the Order domain.
...
- Carriers have **two distinct identifiers**: `CarrierId` (the row primary key, changes on edit because PrestaShop copies the carrier) and `CarrierReferenceId` (stable reference id that persists across edits). Always use `CarrierReferenceId` when linking carriers to other entities across versions.
