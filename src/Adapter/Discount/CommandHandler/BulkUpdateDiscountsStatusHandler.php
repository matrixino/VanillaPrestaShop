...
#[AsCommandHandler]
final class BulkUpdateDiscountsStatusHandler implements BulkUpdateDiscountsStatusHandlerInterface
{
    /**
     * {@inheritdoc}
     *
    ...
    {
        foreach ($command->getDiscountIds() as $discountId) {
            $entity = new CartRule($discountId->getValue());
            $entity->active = $command->getNewStatus();

            if (!$entity->id) {
                throw new DiscountNotFoundException(sprintf('Discount with id "%s" was not found', $discountId->getValue()));
            }

            if (!$entity->update()) {
                throw new CannotUpdateDiscountException(sprintf('Cannot update status for discount with id "%s"', $discountId->getValue()));
            }
        }
    }
}
